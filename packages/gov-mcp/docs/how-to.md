...
> **Status changes are automatically traced.** Every `update_*_status` call creates a `status_history` row and a `log_entry` with `type: "status"`. You do not need to manually post a log entry for status transitions.
