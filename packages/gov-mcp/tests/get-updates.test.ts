...
describe("get_updates, mailbox_last_24h, and spec status", () => {
  ...
  it("get_updates with last returns N most recent entries (DESC) and advances cursor to max", async () => {
    const read = await import("../src/tools/read.js");
    const write = await import("../src/tools/write.js");
    const queries = compileQueries(db);
    const ctx = { db, queries };
    ...
    const nanoid = regResult.structuredContent?.nanoid as string;
    const result = read.getUpdates(ctx, { nanoid, last: 3 });
    expect(result.isError).toBe(false);
    ...
    // new_cursor is now an ISO timestamp
    expect(typeof data.new_cursor).toBe("string");
    // has_more is true (2 older entries exist)
    expect(data.has_more).toBe(true);
  });

  it("get_updates with resetCursor advances cursor to max without returning entries", async () => {
    const read = await import("../src/tools/read.js");
    const write = await import("../src/tools/write.js");
    const queries = compileQueries(db);
    const ctx = { db, queries };
    ...
    const nanoid = regResult.structuredContent?.nanoid as string;
    const result = read.getUpdates(ctx, { nanoid, resetCursor: true });
    expect(result.isError).toBe(false);
    ...
    // new_cursor is now an ISO timestamp
    expect(typeof data.new_cursor).toBe("string");
    // Verify cursor was actually updated in DB
    const writer = queries.getWriterByNanoid.get({ nanoid });
    expect(writer!.last_read_at).not.toBeNull();
  });

  it("get_updates with since returns entries after the given id (DESC)", async () => {
    const read = await import("../src/tools/read.js");
    const write = await import("../src/tools/write.js");
    const queries = compileQueries(db);
    const ctx = { db, queries };
    const regResult = write.registerWriter(ctx, { id: "test-agent-since", role: "agent" });
    const nanoid = regResult.structuredContent?.nanoid as string;
    // Since timestamp — should return entries after that point in DESC order
    const sinceTs = "1970-01-01T00:00:00.000Z";
    const result = read.getUpdates(ctx, { nanoid, since: sinceTs });
    expect(result.isError).toBe(false);
