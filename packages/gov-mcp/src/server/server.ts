  ...
  const server = new McpServer(
    { name: pkg.name, version: pkg.version },
    {
      ...
      instructions: [
        ...
        `2. Read \`get_doc({ filename: \"how-to.md\" })\` — every workflow (register, create, update, correct, handoff)`,
        `3. Ask ADMIN for your scope, role, and responsibilities (questions/answers)`,
        `4. Synthesize the answers and ask ADMIN to validate before registering`,
        `5. Call \`register_me\` with the validated profile — SAVE the returned nanoid (it is shown only once and cannot be retrieved)`,
        `6. Call \`list_docs\` to see available documentation, \`get_doc\` to read any doc`,
        `7. Use \`get_updates\` with your nanoid to read unread log entries`,
      ].join("\n"),
    },
  );
  ...
  for (const tool of toolList) {
    ...
    server.registerTool(
      ...
      async (args: Record<string, unknown>) => {
        // CAST: tool.handler is a union of specific handler types;
        // all handlers accept Record<string, unknown> at runtime (they call .safeParse() internally).
        return toCallToolResult((tool.handler as (ctx: IToolCtx, args: Record<string, unknown>) => OToolResult)(ctx, args));
      },
    );
  }
