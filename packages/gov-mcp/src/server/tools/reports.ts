...
/** Write a Markdown file to the reports directory and return the absolute path. */
function writeReport(ctx: IToolCtx, filename: string, markdown: string): string {
  const filepath = join(ctx.reportsDir, filename);
...
export function generateDailyReport(
  ...
  // Log entries: sliding 24h window (from report generation time backwards)
  const since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  const logEntries = q.reportLogEntriesSince.all({ since });
  ...
  const scopeByAuthor = new Map(writers.map((w) => [w.id, w.default_scope]));
...
Generated by @ytrynot/gov-mcp on ${currentDateTimeUTC()}.
...
## Log
${logEntries.length > 0 ? "\n" + logEntries.map((e) => `### ${formatDateTimeUTC(e.date)} - ${e.type} - ${e.subject ?? "—"}

**From:** ${e.author ?? "—"}
**Scope:** ${e.author ? (scopeByAuthor.get(e.author) ?? "—") : "—"}
**For:** ${e.audience ?? "—"}
...
`;
  ...
  const filepath = writeReport(ctx, filename, md);
...
export function generateDecisionsReport(ctx: IToolCtx): OToolResult {
  const scopesFor = (entityType: string, entityId: string): string => {
    const rows = ctx.queries.getEntityScopes.all({ entity_type: entityType, entity_id: entityId });
    return rows.map((r) => r.scope_id).join(", ") || "—";
  };
...
|----|--------|-------|--------|------|---------|------------|
${rows.map((r) => `| [${r.id}](#${r.id.toLowerCase()}) | ${r.status} | ${r.title} | ${scopesFor("decision", r.id)} | ${formatDateTimeUTC(r.date)} | ${r.decider} | ${supersedesOf(r.id)} |`).join("\n")}
...
export function generateActionsReport(ctx: IToolCtx): OToolResult {
  const scopesFor = (entityType: string, entityId: string): string => {
    const rows = ctx.queries.getEntityScopes.all({ entity_type: entityType, entity_id: entityId });
    return rows.map((r) => r.scope_id).join(", ") || "—";
  };
...
export function generateIdeasReport(ctx: IToolCtx): OToolResult {
  const scopesFor = (entityType: string, entityId: string): string => {
    const rows = ctx.queries.getEntityScopes.all({ entity_type: entityType, entity_id: entityId });
    return rows.map((r) => r.scope_id).join(", ") || "—";
  };
...
export function generateProblemsReport(ctx: IToolCtx): OToolResult {
  const scopesFor = (entityType: string, entityId: string): string => {
    const rows = ctx.queries.getEntityScopes.all({ entity_type: entityType, entity_id: entityId });
    return rows.map((r) => r.scope_id).join(", ") || "—";
  };
...
/** Generate a full specs report as Markdown and write it to disk. */
export function generateSpecsReport(ctx: IToolCtx): OToolResult {
  const scopesFor = (entityType: string, entityId: string): string => {
    const rows = ctx.queries.getEntityScopes.all({ entity_type: entityType, entity_id: entityId });
    return rows.map((r) => r.scope_id).join(", ") || "—";
  };
  const rows = ctx.queries.listSpecs.all() as {
    id: string; filename: string; package: string | null;
    version: number; status: string; date: string;
    supersedes: string | null; created_at: string; updated_at: string;
  }[];

  const md = `# Mailbox — Specs (generated)

Generated by @ytrynot/gov-mcp on ${currentDateTimeUTC()}.

## Index

| ID | Status | Filename | Package | Version | Date | Scopes | Supersedes |
|----|--------|----------|---------|---------|------|--------|------------|
${rows.map((r) => `| [${r.id}](#${r.id.toLowerCase()}) | ${r.status} | ${r.filename} | ${r.package ?? "—"} | v${r.version} | ${formatDateTimeUTC(r.date)} | ${scopesFor("spec", r.id)} | ${r.supersedes ? entityLink(r.supersedes) : "—"} |`).join("\n")}
...
| Version | v${r.version} |
| Date | ${formatDateTimeUTC(r.date)} |
| Scopes | ${scopesFor("spec", r.id)} |
| Supersedes | ${r.supersedes ? entityLink(r.supersedes) : "—"} |
| Created at | ${formatDateTimeUTC(r.created_at)} |
| Updated at | ${formatDateTimeUTC(r.updated_at)} |
---
`).join("\n")}
`;

  const filename = "mailbox-specs.md";
  const filepath = writeReport(ctx, filename, md);

  return ok(`Specs report (${rows.length} specs)`, {
    filename,
    filepath,
    markdown: md,
    count: rows.length,
...
export function generateDecisionHistoryReport(
  ...
  const dec = q.getDecisionById.get({ id: input.id });
...
| Date | ${formatDateTimeUTC(dec.date)} |
| Decider | ${dec.decider} |
...
|------------|------------|------------|------------|--------|
${history.map((h) => `| ${h.old_status ?? "—"} | ${h.new_status} | ${formatDateTimeUTC(h.changed_at)} | ${h.changed_by} | ${h.reason ?? "—"} |`).join("\n")}`}

## Log Entries
${logs.length === 0 ? "_No log entries._" : logs.map((l) => `### [${formatDateTimeUTC(l.timestamp)}] — ${l.type}
...
export function generateAllReports(ctx: IToolCtx): OToolResult {
  const results: { report: string; filepath: string }[] = [];
  const errors: string[] = [];

  const reports: { name: string; fn: (ctx: IToolCtx) => OToolResult }[] = [
    ...
    { name: "specs", fn: generateSpecsReport },
