...
export function help(
  ...
  const lines: string[] = [
    ...
    '- **Edit field**: `edit_field({ entityType, entityId, field, newValue, reason })` — for log_entries use `append_log_entry({ type: "correction", replyTo })`',
    ...
    "",
    "All `create_*` and `update_*_status` calls automatically log a `log_entry` (type `\"status\"` for status changes, entity type for creations) and a `status_history` row. You do not need to manually post a log entry for status transitions — the tool does it for you.",
    "",
    "## Before You Write — Read how-to.md",
    "",
    "**`get_doc({ filename: \"how-to.md\" })`** — covers every workflow: register, create, update status, edit fields, handoff, search. If you are about to create or update an entity and have not read it this session, read it first.",
    "",
    "## Docs",
    "",
