...
describe("B12 — churn (30% of the corpus replaced)", () => {
  it("erodes and prunes edges to removed atoms (traced); orphans cool down", () => {
    ...
    osem.maintain();
    for (let i = 0; i < 10; i++)
      osem.recallShallow({ agentId: "B12", prompt: "churn tokC0 tokC1" });
    ...
    osem.maintain();
    const zombie = osem.recallShallow({ agentId: "B12", prompt: "churn tokC1 tokC2" });
    expect(zombie.filter(x => x.id.startsWith("synth:churn"))).toHaveLength(0);
    // The stable corpus still surfaces.
    const stable = osem.recallShallow({ agentId: "B12", prompt: "coercion rules" });
    expect(stable.length).toBeGreaterThan(0);
  });
});

describe("B13 — crash recovery (kill mid-session, reopen)", () => {
  it("loses working memory (assumed) but the cadastre + salience survive", () => {
    const path = ":memory:";
    ...
    osem.recallShallow({ agentId: "B13", prompt: "tokKEEP durable" });
    const salienceBefore = (db.prepare(
      `SELECT salience, uses FROM atom_sediment
    ...
    // The landscape regenerates: the cadastre + salience are intact.
    const after = db.prepare(
      `SELECT salience FROM atom_sediment
       WHERE atom_id = 'ATOM-KEEP' AND scope_id = 'agent:B13'`).get() as
      { salience: number };
    expect(after.salience).toBeGreaterThan(0);
    // A fresh excitation re-surfaces the consolidated fact.
    const again = osem.recallShallow({ agentId: "B13b", prompt: "tokKEEP durable" });
...
describe("B14 — multi-agents (2 agent_ids in parallel, one cadastre)", () => {
  it("keeps working memories isolated while the lattice stays coherent", () => {
    const { osem, db } = makeOsem();
    osem.recallShallow({ agentId: "alpha", prompt: "maranget" });
    osem.recallShallow({ agentId: "beta", prompt: "serialization toJS" });
