...
| `z.property(key, schema)` (top-level check) | `dna.property(key, schema)` | ✅ |
| `z.properties(shape)` (top-level check) | `dna.properties(shape)` | ✅ (symbol keys unsupported — throws; see below) |
| `.properties(shape)` on `z.instanceof()` | `.properties(shape)` on `dna.instanceof()` | ✅ |
...
| Zod v4 | DNA | Status |
|---|---|---|
| `.meta()` | `.meta()` | ⚠️ Different semantics — Zod's `.meta()` stores arbitrary metadata (mainly for registry/`$defs` hoisting). DNA's `.meta()` is a **functional** metadata channel that drives the runtime: `optional`, `nullable`, `default`, `coerced`, `externals`, `id`, etc. are all `tsDnaInnerMeta` fields serialized directly into the DNA bytecode. Zod separates concepts (`.meta()`, `.describe()`, `.default()`, etc.); DNA unifies them in one serializable channel. |
| `.describe()` | `.describe()` | ✅ |
| `z.registry()` | — | ❌ Not needed — Zod's registry is a write-only `WeakMap` with no `getById` retrieval (`_idmap` is internal, consumed only by `toJSONSchema`). DNA's `.meta({ id })` travels with the schema in the bytecode, is inspectable at any time, and the converter hoists to `$defs` directly. The registry adds complexity for a write-only map with no retrieval API. |
| `z.globalRegistry` | — | ❌ Same reasoning as `z.registry()` |
| `z.config()` | — | ❌ |
...
| `toJSONSchema({ cycles: "ref" })` | Default behavior | ✅ DNA resolves cycles with `$ref`/`$defs` |
| `toJSONSchema({ cycles: "throw" })` | — | ❌ Niche validation guard (throw if recursive). Low practical value; no plan to implement. |
| `toJSONSchema({ registry })` | — | ❌ Not needed — `.meta({ id })` covers the same use case (see section 21). |
| `toJSONSchema({ reused: "ref" })` | Tends toward `ref` by default | 🔍 Needs investigation — DNA already hoists schemas with `.meta({ id })` into `$defs`. Whether DNA should also auto-hoist reused schemas *without* an `id` (like Zod's `reused: "ref"`) requires empirical comparison. |
| `toJSONSchema({ io: "input" })` | — | 🟡 Interesting — would generate the JSON Schema of the input type (before transforms/coerce). Worth exploring. |
| `toJSONSchema({ unrepresentable })` | — | 🟡 Interesting — DNA's bytecode can represent transforms and functions (via `externals`), so the goal would be to give even those a JSON Schema representation. Worth exploring. |
| `toJSONSchema({ override })` | — | 🟡 Interesting — a per-schema callback to customize the generated JSON Schema. Worth exploring. |
...
| `__proto__` declared in schema | **broken** — JIT fastpass throws `SyntaxError` (#4357, #4358); `zod-from-json-schema` #41 strips it from output | **Limited** — `Object.create(null)` is used when `__proto__` is detected in declared keys, but the builder API (`dna.object({ "__proto__": ... })`) cannot declare `__proto__` because JS object literals trigger the `Object.prototype.__proto__` setter, silently dropping the key before DNA sees it. Works only via the JSON Schema → DNA path (schvalid), where `JSON.parse` creates real own properties. |
| JSON Schema Test Suite (`properties.json`, `required.json`) | ❌ fails on declared `__proto__` | ✅ passes via **schvalid** (JSON Schema → DNA path, where `JSON.parse` creates real own properties). Not via the DNA builder API. |
| Runtime overhead | Checks `__proto__` in every loop iteration (catchall/passthrough) | **None** — DNA uses null-prototype objects (`Object.create(null)`) so there's no `__proto__` setter to protect; the check is done at codegen time, not at runtime |
