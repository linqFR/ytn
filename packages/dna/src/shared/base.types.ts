...
import type { tsStateDef, tsStateFull } from "../builder/state.types.js";
import type { StateManager, tsStateMgrInst } from "../builder/state-manager.js";
...
export type $DnaShapeToValue<T> = T extends Record<PropertyKey, any>
  ? { [K in keyof T]: T[K] extends tsDnaType<infer V, any> ? V : T[K] }
  : T;

// Lazy schema interface (for recursive definitions)
export interface IDnaLazySchema<T> extends tsDnaType<T> {
  get innerType(): T;
}

// Variadic sequence types for pipe/seq operations
export type $SeqFirstInput<Schemas extends readonly tsDnaType<any, any>[]> =
  Schemas extends readonly [tsDnaType<any, infer I>, ...any[]] ? I : never;

export type $SeqLastOutput<Schemas extends readonly any[]> =
  Schemas extends readonly [...any[], tsDnaType<infer O, any>] ? O : never;
...
export type $DnaInferOutput<S> = S extends { _output: infer O } ? O : S extends tsDnaType<infer V, any> ? V : never;
export type $DnaInferInput<S> = S extends { _input: infer I } ? I : S extends tsDnaType<any, infer I> ? I : never;
...
export type $ValidatePipeArgs<PrevOutput, Schemas extends readonly any[]> =
  Schemas extends readonly [tsDnaType<infer O, infer I>, ...infer Rest]
  ? [PrevOutput] extends [I]
  ? [tsDnaType<O, I>, ...$ValidatePipeArgs<O, Rest>]
  : [tsDnaType<any, PrevOutput>, ...any[]] // Force une erreur TS explicite si mismatch
  : [];
...
export interface tsDnaType<T = any, I = unknown, S extends tsStateDef = tsStateDef> extends IDnaSchemaCore<T, I> {
  ...
  // _class: $StateDefToClass<S, T, I>;
  _core: tsStateMgrInst<S>;
  ...
  brand<B extends string, D extends "out" | "in" | "inout" = "out">(brand?: B, direction?: D): $WithBrand<this, B, D>;
