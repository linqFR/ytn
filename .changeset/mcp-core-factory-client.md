...
- `startStdioServer` serves the registry over stdio with era-aware protocol negotiation; `publishDiscoveryFile` writes a JSON descriptor for sibling processes (world-readable — never put secrets in `fields`).
