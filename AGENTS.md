...
- **File Encoding**: All files MUST be encoded in **UTF-8 without BOM** (no Byte Order Mark). Never write files in Windows-1252, Latin-1, or any other encoding. When writing files programmatically (e.g., via PowerShell `[System.IO.File]::WriteAllText`), explicitly use `UTF8Encoding($false)` to avoid the BOM. When reading files with PowerShell `Get-Content`, the default encoding is Windows-1252 on Windows — always pass `-Encoding UTF8` or use `[System.IO.File]::ReadAllText($path, [System.Text.UTF8Encoding]::new($false))` to avoid corrupting non-ASCII characters (em-dashes, curly quotes, etc.). For bulk text replacements across multiple files, prefer `node --input-type=commonjs -e` with `fs.readFileSync`/`fs.writeFileSync` (Node defaults to UTF-8) over PowerShell string operations.
